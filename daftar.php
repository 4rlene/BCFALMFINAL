...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($kategori['allow_smp']) {
        $allowedKelas = array_merge($allowedKelas, ['VII','VIII','IX']);
    }
    if ($kategori['allow_sma']) {
        $allowedKelas = array_merge($allowedKelas, ['X','XI','XII']);
    }
    
    if (!in_array($kelas, $allowedKelas, true)) {
        $kelas = $allowedKelas[0] ?? 'VII';
    }
    
    // Validate required fields
    if (empty($nama) || empty($email) || empty($telepon) || empty($sekolah) || empty($kelas)) {
        ...
        if ($result) {
            ...
            if ($jenis_lomba == 'kelompok') {
                // Ensure at least 2 members (including ketua) for kelompok
                $jumlah_anggota = max(2, intval($_POST['jumlah_anggota'] ?? 0));
                ...
                // Client generates (jumlah_anggota - 1) extra members starting from index 1
                for ($i = 1; $i < $jumlah_anggota; $i++) {
                    ...
                    if (!in_array($anggota_kelas, $allowedKelas, true)) {
                        $anggota_kelas = $allowedKelas[0] ?? 'VII';
                    }
                    ...
                    if (!empty($anggota_nama) && !empty($anggota_sekolah) && !empty($anggota_kelas)) {
                        ...
                        // Handle Instagram follow proof uploads for group members
                        $anggota_ig_bluvocationfest_proof = '';
                        $anggota_ig_smkblverse_proof = '';
                        
                        // Upload @bluvocationfest proof for member
                        ...
                        // Upload @smkbl.verse proof for member
                        ...
                        $db->insert($anggotaTable, $anggota_data);
                    }
                }
            }
                    ...
                    <div class="p-4">
                        ...
                        <form method="POST" enctype="multipart/form-data">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select class="form-select" id="kelas" name="kelas" required>
                                            ...
                                            <?php if ($kategori['allow_smp']): ?>
                                                <option value="VII">SMP Kelas 1 (VII)</option>
                                                <option value="VIII">SMP Kelas 2 (VIII)</option>
                                                <option value="IX">SMP Kelas 3 (IX)</option>
                                            <?php endif; ?>
                                            <?php if ($kategori['allow_sma']): ?>
                                                <option value="X">SMA/Sederajat Kelas 1 (X)</option>
                                                <option value="XI">SMA/Sederajat Kelas 2 (XI)</option>
                                                <option value="XII">SMA/Sederajat Kelas 3 (XII)</option>
                                            <?php endif; ?>
                                        </select>
                                    </div>
                                </div>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        <label for="ig_smkblverse_proof" class="form-label">Bukti Follow @smkbl.verse *</label>
                                        <input type="file" class="form-control" id="ig_smkblverse_proof" name="ig_smkblverse_proof" accept="image/*" required>
                                        <small class="text-muted">Screenshot bukti follow Instagram @smkbl.verse</small>
                                    </div>
                                </div>
                            </div>
                            <?php endif; ?>
                ...
                const memberHtml = `
                    <div class="member-section">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    <label class="form-label">Bukti Follow @smkbl.verse *</label>
